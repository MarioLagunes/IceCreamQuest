...
public class MenuPrincipal extends mx.itesm.icecreamquest.PantallaDatos implements Screen {
	private final mx.itesm.icecreamquest.Juego juego;
	...
	public Music musica;
	private mx.itesm.icecreamquest.Musica musicaMenu;
	public static Boolean ajuste = false;
	public Fondo fondoComic;

	public MenuPrincipal(mx.itesm.icecreamquest.Juego juego) {
		this.juego = juego;
	}
	...
	private void cargarFuncionesBontones(ImageButton[] botones1) {
		...
		btonJugar.addListener(new ClickListener(){
			...
			public void clicked(InputEvent event, float x, float y) {
				juego.setScreen(new mx.itesm.icecreamquest.Comic(juego));
				musica.stop();
			}
		});
		...
		btonAcercaDe.addListener(new ClickListener(){
			@Override
			public void clicked(InputEvent event, float x, float y) {
				juego.setScreen(new mx.itesm.icecreamquest.PantallaAcercaDe(juego));
				musica.stop();
			}
		});
		...
		btonPuntaje.addListener(new ClickListener(){
			@Override
			public void clicked(InputEvent event, float x, float y) {
				juego.setScreen(new mx.itesm.icecreamquest.PantallaPuntaje(juego));
				musica.stop();
			}
		});
	...
	private void imagenesFondo() {
		...
		musicaMenu = new mx.itesm.icecreamquest.Musica(musica,true,ajuste);
	}
